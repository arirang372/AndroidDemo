...
public class AceDashboardRepository
    ...
    private void considerTrackingNextStepsDisplayed() {
        if (!isAnyRequestInFlight()) {
            new AceNextStepActionTrackingHelper(getAnalyticsFacade()).considerTrackingNextStepsDisplayed(getModel().getDashboardCards());
        }
    }
    ...
    public void considerTriggeringAirshipEvents() {
    ...
    @SuppressWarnings("WeakerAccess")
        // to avoid auto-generated accessor
    AceDashfolioFlow getDashfolioFlow() {
    ...
    public AceDashboardStateEmitter getStateEmitter() {
        return (AceDashboardStateEmitter) super.getStateEmitter();
    }
...
// re-implement in this package so AcePolicySessionEstablisherFromSelector has access
    ...
    private void logDisplayOfBillingQuickActionCard() {
        logEvent(getModel().getPayMyBillCardText().equals(getString(R.string.payMyBill)) ?
                M5_DASHBOARD_QUICK_ACTION_PAY_BILL_DISPLAYED : M5_DASHBOARD_QUICK_ACTION_VIEW_BILL_DISPLAYED);
    }

    public void logDisplayedDashboardMetrics() {
        logDisplayOfBillingQuickActionCard();
    ...
    public void logWhatIsNextCardSelected(@DashboardCardType String cardType) {
        ...
        new AceNextStepActionTrackingHelper(getAnalyticsFacade()).considerTrackingNextStepSelected(getTrackable(), cardType, cardIndex);
    ...
    public void makePayment() {
        ...
        if (getPolicy().getAutomaticPaymentDetails().getStoredAccount().isCardExpired()) {
    ...
    private void navigateToErsRequestConfirmation(Context context, AceClaimAlertNotification alertNotification) {
    ...
    public void openDialog(@NonNull @DialogTag String dialogTag) {
        switch (dialogTag) {
            case AceConstantState.DialogTag.ENROLL_WITH_DEVICE_UNLOCK:
                getStateEmitter().emitEnrollWithDeviceUnlockDialogVisibility(Visibility.VISIBLE);
                break;
    ...
    private void setClaimsAlertNeedRefreshing() {
