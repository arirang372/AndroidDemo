...
public abstract class AceTierRepository<M extends AceViewBackingModel> extends AceBaseRepository<M> implements
    ...
    public boolean userWasEnrolledForDeviceUnlock() {
        String userId = getLoginSettingsDao().getLastUserEnteredId();
        String fingerprintUserId = getLoginSettingsDao().getLastFingerprintUserEnteredId();
        return !fingerprintUserId.isEmpty() && !userId.equalsIgnoreCase(fingerprintUserId);
    }
    ...
    public String getMessageForDeviceUnlockDialog() {
        ...
        if (userWasEnrolledForDeviceUnlock()) {
            return getApplication().getString(R.string.enrollForScreenUnlockDescriptionChangeUser, fingerprintUserId, userId);
        }
    ...
    private void recordMetricsOnDeviceUnlockSelected() {
        ...
        String contextVariable = userWasEnrolledForScreenUnlock ?
                SCREEN_UNLOCK_ENROLLMENT_CHANGE_YES :
                SCREEN_UNLOCK_ENROLLMENT_YES;
        String actionValue = userWasEnrolledForScreenUnlock ?
                ANALYTICS_SCREEN_UNLOCK_ENROLLMENT_CHANGE_USER_YES :
                ANALYTICS_SCREEN_UNLOCK_ENROLLMENT_YES;
    ...
    private void recordMetricsOnDeviceUnlockDiscarded() {
        ...
        String contextVariable = userWasEnrolledForScreenUnlock ?
                SCREEN_UNLOCK_ENROLLMENT_CHANGE_USER_NO :
                SCREEN_UNLOCK_ENROLLMENT_NOT_NOW;
        String actionValue = userWasEnrolledForScreenUnlock ?
                ANALYTICS_SCREEN_UNLOCK_ENROLLMENT_CHANGE_USER_NO :
                ANALYTICS_SCREEN_UNLOCK_ENROLLMENT_NOT_NOW;
        trackAction(actionValue, contextVariable);
    }
