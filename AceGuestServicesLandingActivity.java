...
import com.geico.mobile.android.ace.geicoappbusiness.ui.analytics.trackable.AceTelematicsEnrolledQuestionTrackable;
...
import com.geico.mobile.android.ace.geicoappmodel.enums.telematics.AceTelematicsFeatureModeVisitor;
...
public class AceGuestServicesLandingActivity extends AceBaseGuestServicesFirstStartActivity {
    ...
    private boolean isTelematicsInitializedWithSession() {
        return AceBasicTelematicsSessionManager.INSTANCE.isInitializedWithSession();
    }
    ...
    public void onActivateAccountYesClicked(View view) {
        ...
        considerDisplayingTelematicsEnrolledPage();
    }

    private void considerDisplayingTelematicsEnrolledPage() {
        featureConfiguration.getModeForTelematics().acceptVisitor(new AceTelematicsFeatureModeVisitor<Void, Void>() {
            @Override
            ...
            @Override
            public Void visitInitial(Void input) {
                trackPageShown(new AceTelematicsEnrolledQuestionTrackable(analyticsFacade));
                publish(AceGuestServicesFlow.GUEST_SERVICES_EVENT_TELEMATICS_ENROLLMENT_QUESTIONS);
                return NOTHING;
            }

            @Override
            public Void visitSdkOff(Void input) {
                navigateToLoginPage();
                return NOTHING;
            }
        });
    ...
    protected void onStart() {
        super.onStart();
        if ("".equals(loginSettingsDao.retrieveUserId()) && !isTelematicsInitializedWithSession()) {
            return;
        }
    ...
    public void onTelematicsEnrollmentYesClicked(View view) {
        ...
        trackAction(ANALYTICS_TELEMATICS_ENROLLED_QUESTION_YES_SELECTED, TELEMATICS_ENROLLED_QUESTION_YES_SELECTED);
    ...
    protected void wireUpDependencies(AceRegistry registry) {
        ...
        analyticsFacade = registry.getAnalyticsFacade();
        branchLinkHandler = registry.getBranchLinkHandler();
        featureConfiguration = registry.getFeatureConfiguration();
