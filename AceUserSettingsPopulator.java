...
public class AceUserSettingsPopulator extends AceBaseUserSettingsPopulator {
    ...
    protected List<AceUserSettingsSectionItem> buildUserSettingsSectionItems(AceRegistry source, AceUsersModel target) {
        ...
        sectionItems.add(createUserSettingsSectionItemWith(resourceConverter.convert(R.string.name), profilePerson.getFullName(), true, false, false, null));
        populatePhoneNumberSection(sectionItems, profilePerson, target);
        sectionItems.add(createUserSettingsSectionItemWith(resourceConverter.convert(R.string.mailingAddress), getTrimmedAddress(policy.getContact().getMailingAddress().getDisplayableMailingAddress()),
                true, false, shouldShowEditAddressButton || isSpecialtyVehicle,
                shouldShowEditAddressButton ? source.getApplicationContext().getDrawable(R.drawable.ic_edit_pencil) : isSpecialtyVehicle ? source.getApplicationContext().getDrawable(R.drawable.ic_phone) :
                        null));
        return sectionItems;
    }
    ...
    private void populatePhoneNumberSection(List<AceUserSettingsSectionItem> sectionItems, AceUserProfilePerson profilePerson, AceUsersModel target) {
        AceUserSettingsSectionItem phoneNumberSection = createUserSettingsSectionItemWith(resourceConverter.convert(R.string.mobilePhone), profilePerson.getMobilePhoneNumber(), false, true, false, null);
