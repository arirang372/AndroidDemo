...
public class AceLoginSettingsRepository extends AceTierRepository<AceLoginSettingsModel>
    ...
    private void displayPasswordStrengthMeter(AceLoginSettingsSectionDetailItem passwordFooterSection) {
        AceSimpleRuleEngine.DEFAULT.applyFirst(new AceLoginSettingsPasswordStrengthMeterRuleFactory(passwordFooterSection, getPolicy().getContact().getEmailAddress(),
                        getPolicy().getNumber(), getLoginSettingsFlow().getUserId()).create(),
    ...
    public void initializeModel(@NonNull String viewTag) {
        switch (viewTag) {
            ...
            case ViewTag.LOGIN_SETTINGS_PASSWORD_FRAGMENT:
                getModel().setSelectedSectionItem(findLoginSettingsSectionItemWith(EDIT_PASSWORD));
                getSectionDetailItemWith(EDIT_PASSWORD, 0, 1).getText()
    ...
    private boolean isValidPasswordAndHint() {
        ...
        validatePasswordField(currentPasswordSection, R.string.currentPasswordRequiredText);
        validatePasswordField(newPasswordSection, R.string.newPasswordRequired);
    ...
    private void validatePasswordField(AceLoginSettingsSectionDetailItem passwordSection, @StringRes int blankPasswordErrorMessageId) {
        AceSimpleRuleEngine.DEFAULT.applyFirst(new AceLoginSettingsPasswordValidationRuleFactory(passwordSection, this::getString,
