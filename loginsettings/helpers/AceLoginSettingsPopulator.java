...
public class AceLoginSettingsPopulator implements AcePopulator<AceRegistry, AceLoginSettingsModel> {
    ...
    private List<AceLoginSettingsSectionItem> buildLoginSettingsSectionItems(AceUserLoginSettingsFlow loginSettingsFlow) {
        List<AceLoginSettingsSectionItem> sectionItems = new ArrayList<>();
        sectionItems.add(createLoginSettingsSectionItemWith(resourceConverter.convert(R.string.userIdText), loginSettingsFlow.getUserId(), EDIT_USER_ID));
        AceLoginSettingsSectionItem passwordSectionItem = createLoginSettingsSectionItemWith(resourceConverter.convert(R.string.passwordText), EDIT_PASSWORD);
        new AceLoginSettingsPasswordSectionPopulator(resourceConverter).populate(loginSettingsFlow, passwordSectionItem);
        sectionItems.add(passwordSectionItem);
        AceLoginSettingsSectionItem securityQuestionSectionItem = createLoginSettingsSectionItemWith(resourceConverter.convert(R.string.securityQuestions), EDIT_SECURITY_QUESTION);
        new AceLoginSettingsSecurityQuestionSectionPopulator(resourceConverter).populate(loginSettingsFlow, securityQuestionSectionItem);
        sectionItems.add(securityQuestionSectionItem);
        return sectionItems;
    }
    ...
    private boolean isFingerprintLoginAvailable(AceLoginSettingsDao loginSettingsDao) {
    ...
    private boolean isScreenUnlockEnabled(AceRegistry source) {
        ...
        return isFingerprintLoginAvailable(loginSettingsDao) || loginSettingsDao.isEnrolledForDeviceUnlock();
    }
    ...
    public void populate(@NonNull AceRegistry source, @NonNull AceLoginSettingsModel target) {
        ...
        target.getLoginSettingsListItems().add(createLoginSettingsListItemWith(resourceConverter.convert(R.string.loginSettings),
                buildLoginSettingsSectionItems(source.getSessionController().getPolicySession().getLoginSettingsFlow())));
    }
