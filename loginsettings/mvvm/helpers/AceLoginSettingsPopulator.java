...
public class AceLoginSettingsPopulator implements AcePopulator<AceRegistry, AceLoginSettingsModel> {
    ...
    private void considerPopulatingScreenUnlockRow(AceRegistry source, AceLoginSettingsModel target) {
        if (shouldShowScreenUnlockSettings(source)) {
            boolean isScreenUnlockEnabled = isScreenUnlockEnabled(source);
            target.getLoginSettingsListItems().add(createLoginSettingsListItemWith(resourceConverter.convert(R.string.screenLockSettings),
                    Arrays.asList(createLoginSettingsSectionItemWith(resourceConverter.convert(R.string.enableScreenUnlock), NOT_APPLICABLE)
                    ...
                    )).withSubtitle(getScreenUnlockSettingsSubtitle(source)));
        }
    }
    ...
    private boolean isScreenUnlockEnabled(AceRegistry source) {
        return new AceLoginSharedPreferencesDao(source).isTouchIdEnabled();
    }
